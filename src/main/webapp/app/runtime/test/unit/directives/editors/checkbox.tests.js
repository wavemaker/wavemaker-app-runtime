...
describe("\nUnit: Testing Editor Directives: CheckBox", function () {
    'use strict';
    var $compile, $rootScope, element, scope, dirScope;

    beforeEach(WM.mock.module('wmCore'));

    beforeEach(WM.mock.inject(function ($compile, $rootScope) {
        element = WM.element('<wm-checkbox></wm-checkbox>');
        $compile(element)($rootScope);
        $rootScope.$digest();
        scope = $rootScope;
        dirScope = element.scope();
    }));
    ...
    it("should default dataCheckValue property is to be FALSE", function () {
        expect(element.scope().dataCheckValue).toBeFalsy();
    });
    ...
    it("should caption width change to checkBoxWidget ", function () {
        element.scope().caption = "checkBoxWidget";
        element.scope().$apply();
        expect(element.find('label').text()).toBe("checkBoxWidget");
    });
    ...
    it("should checkbox must be checked when the dataCheckValue is TRUE", function () {
        element.scope().dataCheckValue = true;
        element.scope().$apply();
        expect(element.find('input[type="checkbox"]').prop('checked')).toBeTruthy();
    });
    it("should checkbox must be invisible when the editorBorder is FALSE", function () {
        element.scope().editorBorder = false;
        element.scope().$apply();
        expect(element.find('input[type="checkbox"]').hasClass('no-border')).toBeTruthy();
    });
