...
describe("\nUnit: Testing Editor Directives: TextArea", function () {
    'use strict';
    ...
    beforeEach(WM.mock.inject(function ($compile, $rootScope) {
        element = WM.element('<wm-textarea></wm-textarea>');
    ...
    it("should default captionAlign property is to be left", function () {
        expect(element.scope().captionAlign).toBe("left");
    });
    ...
    it("should caption name change to textAreaWidget ", function () {
        element.scope().caption = "textAreaWidget";
        element.scope().$apply();
        expect(element.find('label').text()).toBe("textAreaWidget*");
    });
