...
describe("\nUnit: Testing Editor Directives: Select", function () {
    'use strict';
    ...
    beforeEach(WM.mock.inject(function ($compile, $rootScope) {
        element = WM.element('<wm-select></wm-select>');
    ...
    it("should display an empty dropdown", function () {
    ...
    it("should default caption property is to be Label", function () {
        expect(element.scope().caption).toBe('selectMenu');
    });
    it("should default captionSize property is to be 75", function () {
        expect(element.scope().captionSize).toBe(75);
    });

    it("should default options property to be undefined", function () {
        expect(element.scope().options).toBe('');
    });
    ...
    it("should caption name change to selectBoxWidget ", function () {
        element.scope().caption = "selectBoxWidget";
        element.scope().$apply();
        expect(element.find('label').text()).toBe("selectBoxWidget*");
    });
    ...
    it("should work options binding When we give the optionsInput string seperated by commas", function () {
        element.scope().options = 'test1,test2,test3';
        element.scope().$apply();
        expect(element.find('select').children('option').length).toBe(3);
    });
