 ...
 */
public class WMSAMLLogoutFilter extends LogoutFilter {
     ...
     */
    public WMSAMLLogoutFilter(String successUrl, LogoutHandler[] localHandler, LogoutHandler[] globalHandlers) {
     ...
     * @param globalHandlers       handlers to be invoked when global logout is selected
     */
    public WMSAMLLogoutFilter(LogoutSuccessHandler logoutSuccessHandler, LogoutHandler[] localHandler, LogoutHandler[] globalHandlers) {
    ...
    protected boolean isGlobalLogout(HttpServletRequest request, Authentication auth) {
        ...
        return (localLogout == null || !"true".equals(localLogout.toLowerCase().trim())) && (getSamlCredentials() instanceof SAMLCredential);
    }
    ...
    private SAMLCredential getSamlCredentials() {
        ...
        if (authentication instanceof WMAuthentication) {
            ...
            return (SAMLCredential) wmAuthentication.getAttributes().get(com.wavemaker.runtime.security.provider.saml.SAMLConstants.SAML_CREDENTIALS).getValue();
        }
        return null;
    }
}
